...
const SheetsDetails = ({ route }) => {
  const { selectedRowData, projectId ,status} = route.params;
  ...
  const handlePrevious = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Unable to load previous data");
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.projectDetails}>
        ...
        <Text style={styles.infoText}>
          ...
          <Text style={{ fontWeight: "bold" }}>{route.params.status}</Text>
        </Text>
      ...
      <View
        style={[
          ...
          {
            height:
              comments.length > 0 ? Math.min(comments.length * 120, 400) : 0,
          },
        ]}
      >
        <ScrollView>
