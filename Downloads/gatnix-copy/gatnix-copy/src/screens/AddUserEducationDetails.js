...
const AddUserEducationDetails = ({ route }) => {
 ...
 const [showStartDatePicker, setShowStartDatePicker] = useState(false);
  ...
  const onEndDateChange = (event, selectedDate) => {
    ...
    setEducation({
      ...education,
      endDate: currentDate.toISOString().split('T')[0], // Format to YYYY-MM-DD
      endDate_Error: false,
      endDate_ErrorText: '',
    });
  };
      ...
      if (["approvalManagers", "super-admin", "employee"].includes(userRole)) {
        ...
        if (mode === "edit_edu" && edu?.id) {
          ...
          response = await axios.post(
            `https://www.gatnix.com/api/v1/org/${orgId}/user/${userId}/user-profile/eductionDetails/association/${userRole}`,
            requestData,
            { headers: { "Content-Type": "application/json" } }
          );
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollViewContainer}>
        <View style={styles.mainContainer}>
          <Text style={styles.headerText}>
            {edu?.id ? "Update Education Details" : "Add Education Details"}
          </Text>
          ...
          <TextInput
            style={[
              styles.input,
              education.foundation_Error && styles.errorInput,
            ]}
          ...
          />
          {education.foundation_Error && (
            <Text style={styles.errorText}>
              {education.foundation_ErrorText}
            </Text>
          )}
          ...
          <TextInput
            style={[
              styles.input,
              education.qualification_Error && styles.errorInput,
            ]}
          ...
          />
          {education.qualification_Error && (
            <Text style={styles.errorText}>
              {education.qualification_ErrorText}
            </Text>
          )}
      ...
      <TextInput
        style={[
          styles.input,
          education.startDate_Error && styles.errorInput,
        ]}
      ...
      />
      {education.startDate_Error && (
        <Text style={styles.errorText}>{education.startDate_ErrorText}</Text>
      )}
      ...
      <TextInput
        style={[
          styles.input,
          education.endDate_Error && styles.errorInput,
        ]}
        placeholder="YYYY-MM-DD"
        keyboardType="numeric"
      ...
      />
      {education.endDate_Error && (
        <Text style={styles.errorText}>{education.endDate_ErrorText}</Text>
      )}
      ...
      {showEndDatePicker && (
        ...
        />
          ...
          <TextInput
            style={[
              styles.input,
              education.location_Error && styles.errorInput,
            ]}
          ...
          />
          {education.location_Error && (
            <Text style={styles.errorText}>{education.location_ErrorText}</Text>
          )}
          <View style={{ alignSelf: "center" }}>
            <TouchableOpacity style={styles.submitButton} onPress={handleSave}>
              <Text style={styles.submitButtonText}>
                {edu?.id ? "Update Details" : "Save Details"}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
